...
# 参数初始化
model_num = rospy.get_param("~model_num")
model_path = rospy.get_param("~model_path")
rospy.loginfo("The number of Model is:" + str(model_num))
rospy.loginfo("The path of Model is:" + model_path)
tcpClients = []
LABELS = []
f = open(model_path, 'r')
for i in range(model_num):
    config_path = f.readline()
    ff = open(config_path, 'r')
    xml_path = ff.readline().split("\n")[0]
    bin_path = ff.readline().split("\n")[0]
    host = ''
    port = int(ff.readline().split("\n")[0])
    address = (host, port)
    tcpClient = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcpClient.connect(address)
    rospy.loginfo("Conect succeed!")
    tcpClients.append(tcpClient)
    LABEL = ff.readline().split("\n")[0].split(" ")
    LABELS.append(LABEL)
    ff.close()
f.close()
...
model_infer_flag = []
for i in range(model_num):
    model_infer_flag.append(False)
color_image = 0
result_data = []
def image_distribute(id):
    rospy.loginfo("Image distribute thread start: " + str(id))
    global color_image
    while True:
        if(model_infer_flag[id]):
            bytedata = color_image.data
            flag_data = str(len(bytedata)) + "," + str(color_image.header.seq)
            tcpClients[id].send(flag_data)
            data = tcpClients[id].recv(1024)
            if(b"ok" == data.decode()):
                tcpClients[id].send(bytedata)
            data = tcpClients[id].recv(1024)
            result_data.append([id, data])
            model_infer_flag[id] = 0
distribute_thread_list = []
for i in range(model_num):
    t = threading.Thread(target=image_distribute, args=(i,))
    t.start()
    distribute_thread_list.append(t)
...
while not rospy.is_shutdown():
    if(len(color_buffer) > 0 and len(depth_buffer) > 0 and new_color_flag and new_depth_flag):
        ...
        # 发送图像
        for i in range(model_num):
            model_infer_flag[i] = True
        while np.array(model_infer_flag, dtype="uint8").sum() != 0:
            pass
        ############################################################
        ...
        obj_list = []
        print(result_data[i])
        results = result_data[i][1].decode().split(" ")
        ...
        if(int(results[0]) > 0):
            ...
            for i in range(int(results[0])):
                ...
                print(LABELS[result_data[i][0]][obj.class_id])

                # 只处理部分结果
                if(LABELS[result_data[i][0]][obj.class_id] != "bottle"):
                    continue
                ###### 深度分析 #####
                # 截取目标物区域
                ...
                if(isNewObj):
                    ...
                    marker.ns=LABELS[result_data[i][0]][obj.class_id] + "_" + str(len(obj_tag_list)); #在标签后添加一个id
