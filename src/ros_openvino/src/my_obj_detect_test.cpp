...
#include <ros_openvino/BBox.h>
#include <ros_openvino/Object.h>
#include <ros_openvino/ObjectArrayStamped.h>
#include <ros_openvino/ObjectAttr.h>
...
ros_openvino::ObjectArrayStamped results_list;
...
int main(int argc, char **argv){
    try{
        ...
        if (output_as_list){
            result_pub = n.advertise<ros_openvino::ObjectArrayStamped>("/object_detection/results",1);
        }
	ros::Publisher ts_hint_pub = n.advertise<std_msgs::Header>("/object_detection/ts_hint", 1);
		...
		int cnt = 0;
			    ...
			    cnt ++;
                            // 大于阈值则认为是一个目标，将它加入已识别目标的列表
                            if (output_as_list){
                                tmp_object.bbox.label=text;
                                tmp_object.bbox.cls_name=text;
                                tmp_object.bbox.cls_id=5;

                                tmp_object.bbox.prob=result_confidence;
                                tmp_object.bbox.top_x=result_xmin * color_width;
                                tmp_object.bbox.top_y=result_ymin * color_height;
                                tmp_object.bbox.width=(result_xmax-result_xmin) * color_width;
                                tmp_object.bbox.height=(result_ymax-result_ymin) * color_height;

                                tmp_object.obj_attr.color = color;
                                tmp_object.obj_attr.area = 0.0;
                                tmp_object.obj_attr.area_label = "small";

                                results_list.data.push_back(tmp_object);
                ...
                // 发布结果话题
                if (output_as_list && cnt > 0){
                    results_list.header.stamp=ros::Time::now();
		    results_list.header.frame_id = "ssd";

		    std_msgs::Header hint = results_list.header;
		    ts_hint_pub.publish(hint);
	
                    result_pub.publish(results_list);
                    results_list.data.clear();
