...
class Objection:
    ...
    def isSameObjection(self, cls_id, x, y, z):
        ...
        if(dist < 0.4):#vector_dist_threshold[self.cls_id]):
            # 如果是同一个物体，则更新当前物体的位置
            return True
        return False
obj_tag_list = []
...
rospy.loginfo("The path of Model config is:" + model_config_path)
...
ff.close()
host = ''
port = int(f.readline().split("\n")[0])
address = (host, port)
tcpClient = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
time.sleep(1)
tcpClient.connect(address)
rospy.loginfo("Conect succeed!" + " Port: " + str(port))
...
pub_image = rospy.Publisher("/object_detection/output_image/compressed", CompressedImage, queue_size=10)
pub_newobj_flag = rospy.Publisher("/object_detection/flag",  Marker, queue_size=10)
markers = MarkerArray()
marker = Marker()
...
while not rospy.is_shutdown():
    if(len(color_buffer) > 0 and len(depth_buffer) > 0 and new_color_flag and new_depth_flag):
        ...
        if(int(results[0]) > 0):
            ...
            for i in range(int(results[0])):
                ...
                if(isNewObj):
                    ...
                    marker.scale.z = 0.3 #box_height

                    # 可乐 果粒橙 雪碧
                    colors = [[1, 0, 0], [1, 1, 0], [0, 1, 0]] # 红 黄 绿
                    marker.color.r = colors[obj.class_id][0]
                    marker.color.g = colors[obj.class_id][1]
                    marker.color.b = colors[obj.class_id][2]
                    marker.color.a = 0.6
                    # global markers
        ...
        bridge = CvBridge()
        pub_image.publish(bridge.cv2_to_compressed_imgmsg(img))
