...
while not rospy.is_shutdown():
    if(color_buffer.count > 0 and depth_buffer.count > 0):
        ...
        while(abs(depth_image.header.stamp.to_sec() - color_image.header.stamp.to_sec()) > 0.03):
            ...
            if(color_buffer_copy.count == 0 and depth_buffer_copy.count == 0):
                print("ok0")
                break
        ...
        if(int(results[0]) > 0):
            ...
            while(buffer.color_image.header.seq + 10 < int(results[1])):
                if(data_buffer_queue.empty()):
                    break
                buffer = data_buffer_queue.get()
                print(buffer.color_image.header.seq)
