...
## 关闭fast_mapping节点
p = subprocess.Popen("ps -aux | grep fast_mapping/fast_mapping_node", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
p.wait()
result = p.stdout.readline().decode('utf-8').split(" ")
pid = list(filter(None, result))[1]
os.system("kill " + pid)
time.sleep(1)
print("fast_mapping shutdown ok")
